...
//     myAccount: IGenericAccount | null = null;

//     async componentDidMount() {
//         this.reader = await Ylide.instantiateReader(
//             EverscaleReadingController,
//             {
//                 dev: true,
//             }
//         );
//         this.sender = await Ylide.instantiateSender(
//             EverscaleSendingController,
//             {
//                 dev: true,
//             }
...
//         this.myAccount = await this.sender.getAuthenticatedAccount();
//     }

//     async authenticate() {
//         const me = (await this.sender.getAuthenticatedAccount())!;
//         const storage = new BrowserLocalStorage();
//         this.keystore = new YlideKeyStore(storage, {
//             onPasswordRequest: async (reason) => {
//                 return prompt(`Enter password for "${reason}"`);
//             },
//             onDeriveRequest: async (reason, magicString) => {
//                 try {
//                     return this.sender.deriveMessagingKeypair(magicString);
//                 } catch (err) {
//                     return null;
//                 }
//             },
...
//             const passwordForKey = prompt(
//                 `Enter password for creating first key`
//             );
//             if (!passwordForKey) {
//                 return;
//             }
...
//         } else {
//             const key = this.keystore.get("everscale", me.address);
//             if (!key || key.address !== me.address) {
//                 throw new Error("Key is fucked up");
//             }
//             const myRegisteredPublicKey =
//                 await this.reader.extractPublicKeyFromAddress(me.address);
//             if (
//                 !myRegisteredPublicKey ||
//                 new SmartBuffer(myRegisteredPublicKey).toHexString() !==
//                     new SmartBuffer(key.publicKey).toHexString()
//             ) {
//                 alert(
//                     "Your public key is not registered to this address. Registering"
//                 );
//                 await this.sender.attachPublicKey(key.publicKey);
//                 alert("Registered");
//             }
//         }
//     }
...
//             //     "result: ",
//             //     await (
//             //         this.reader as EverscaleReadingController
//             //     ).mailerContract.getMsgId(
//             //         keypair.publicKey,
//             //         Math.floor(Math.random() * 100000)
//             //     )
//             // );
//             const recipientPublicKey =
//                 await this.reader.extractPublicKeyFromAddress(recipient);
//             if (!recipientPublicKey) {
//                 throw new Error("Recipient public key not found");
//             }
//             await this.sender.sendMessage([0, 0, 0, 1], keypair, content, [
//                 {
//                     address: recipient,
//                     publicKey: recipientPublicKey,
//                 },
...
//     async readEmails() {
//         const me = (await this.sender.getAuthenticatedAccount())!;
//         const results = await this.reader.retrieveMessageHistoryByDates(
//             me.address
//         );
...
//         return (
//             <div>
//                 <button onClick={() => this.writeEmail()}>write email</button>
//                 <button onClick={() => this.readEmails()}>read emails</button>
//             </div>
//         );
//     }
// }
